...
export default function DashboardHorizontalSection({ title, children }) {
    ...
    return (
        <div className="bg-cradle3 p-4 bg-opacity-20 backdrop-filter backdrop-blur-lg rounded-xl">
            ...
            >
                {title}
                {expanded?<NavArrowUp className="text-inherit" height="1.5em" width="1.5em"/> :<NavArrowDown className="text-inherit" height="1.5em" width="1.5em"/>}
            </div>
            <div
            ...
            >
                <div className="flex flex-wrap gap-2">
                    {children?.length ? children : <div className="text-zinc-500">No items to display</div>}
                </div>
            </div>
        </div>
    );
