 ...
 * @constructor
 */
export default function FleetingNotesPanel({handleFleetingNotesButton,fleetingNotesRefresh}) {
    const auth = useAuth();
    ...
    useEffect(() => {
        getFleetingNotes(auth.access).then(
            response => {
            ...
            displayError(setAlertText, setAlertColor)
        )
    }, [fleetingNotesRefresh]);

    return (
        <>
            <AlertDismissible alert={alertText} setAlert={setAlertText} color={alertColor}/>
            <div className="bg-gray-2 w-full h-full p-4 flex flex-col space-y-2 overflow-hidden" data-testid='fleeting-notes-panel'>
                <div className="h-fit w-full flex flex-row justify-between cursor-pointer"
                     onClick={handleFleetingNotesButton} data-testid='close-fleeting-notes'>
                    <Notes className="text-zinc-500" width="1.5em" height="1.5em"/>
                    <Xmark className="text-zinc-500" width="1.5em" height="1.5em"/>
                </div>
                <div className="w-full h-full overflow-y-auto">
                    {notes && notes.length > 0
                        ? notes.map(note => <FleetingNoteCard note={note} setAlert={setAlertText} setAlertColor={setAlertColor}/>)
                        : <p className="w-full p-2 text-zinc-500 text-center">No notes to display</p>}
                </div>
            </div>
        </>
    );
