 ...
 * @constructor
 */
export default function SearchFilterSection({ showFilters, setShowFilters, entryTypeFilters, setEntryTypeFilters, entityTypeFilters, setEntityTypeFilters }) {
    const toggleFilters = () => {
        setShowFilters(!showFilters);
    };
    return (
        <>
            <div className="flex items-center justify-start cursor-pointer" onClick={toggleFilters}>
                <div className="text-gray-700 font-medium">Filters</div>
                <NavArrowDown className="text-gray-700" height="1.5em" width="1.5em"/>
            </div>
            <div className={`flex-shrink-0 overflow-x-hidden overflow-y-scroll no-scrollbar backdrop-blur-lg rounded-lg my-2 transition-all duration-300 ease-in-out ${showFilters ? 'h-48' : 'h-0 opacity-0'}`}>
                    ...
                    <div className="w-auto flex flex-col">
                        ...
                        <div className="flex flex-wrap">
                            {[...entityCategoriesReduced].map((entityType) => (
                                <SearchFilter option={entityType} filters={entityTypeFilters} setFilters={setEntityTypeFilters}/>
                            ))}
                        </div>
                    </div>
                    <div className="w-auto flex flex-col">
                        ...
                        <div className="flex flex-wrap">
                            {[...entryTypes].map((entryType) => (
                                <SearchFilter option={entryType} filters={entryTypeFilters} setFilters={setEntryTypeFilters}/>
                            ))}
                        </div>
                    </div>
                </div>
