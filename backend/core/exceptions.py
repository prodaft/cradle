...
class CradleAPIException(APIException):
    ...
    def __init__(self, detail=None, code=None, error_code: ErrorCode = None):
        """
        Initialize the exception.

        Args:
            detail: Specific details about this occurrence of the error (overrides default)
            code: Legacy parameter for compatibility (use error_code instead)
            error_code: ErrorCode enum value for this exception (overrides class-level)
        """
        # Use provided error_code or fall back to class-level error_code
        if error_code:
            self.error_code = error_code

        # Set status code from error_code if available
        if self.error_code:
