...
const saveNumber = (number) => {
    return authAxios({ // Note that authAxios has its base URL as the backend API's
        method: 'POST',
        url: '/numbers/',
    });
};
...
export default function RandomNumber() {
    ...
    return (
        <div>
            ...
            <button className='btn btn-primary' onClick={handleSaveNumber}>Save This Number</button>
                ...
                <Routes>
                    <Route element={<PrivateRoute fallback={'/login'} />}>
                        ...
                        <Route path='/' element={<Home />}>
                            ...
                            {/* ... */}
                            <Route path='/random-number' element={<RandomNumber />} /> {/* <-- We will add it here. */}
                        </Route>
...
describe('RandomNumber', () => {
    ...
    it('calls saveNumber with the current number when the save button is clicked', async () => {
        numberService.saveNumber.mockResolvedValueOnce({ message: 'Number saved successfully!' });
