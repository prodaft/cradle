...
threat analysts working on the same cases to centralize and share information. This is
done by allowing analysts to create and share notes referencing specific cases and entries
of interest. 

The application automatically links the entries and cases mentioned in the
notes, making it easier to navigate and understand the context of the information.

CRADLE allows visualizing the relationships between cases and entries through informative
...
[notifications panel](#notifications) and logout controls. In case the user is an administrator,
...
that toggles the display of the fleeting notes panel, and a [search bar](#search) that allows users to search for specific cases, entries, or notes.
...
It provides an overview of the recent activity in the system, including new notes, and recently mentioned cases and actors.

This page can be quickly accessed by clicking on the Home icon in the sidebar.

### Note Taking
CRADLE allows users to create and manage notes related to specific cases and entries.

When a note is created, the system automatically links the cases, actors, and entries mentioned in the note.
These connections are displayed in the [entity dashboards](#dashboards) and [graph visualization](#graph-visualization) features of the application.
...
#### Note Syntax
The note syntax is based on the [original markdown specification](https://daringfireball.net/projects/markdown/syntax) with some additional features to support linking to cases, actors and entries.
The syntax for linking to cases, actors and entries is as follows:
- `[[case:case_name|alias]]` - Link to a case with the specified name and optional alias.
- `[[actor:actor_name|alias]]` - Link to an actor with the specified name and optional alias.
- `[[entry_type:entry_name|alias]]` - Link to an entry with the specified name and optional alias.
...
Files uploaded to notes are not linked to cases, actors or entries, but can be referenced in the note text.
...
- The fleeting notes are not linked to cases, actors or entries and are not visible to other users.
...
- Continue editing the draft note until you are ready to mark it as a final note.
  ...
  When a note is marked as final it becomes immutable, links between entities are created and the note becomes visible to other users.

### Dashboards
Dashboards are pages that provide all the aggregated data related to a specific entity, such as a case, actor, or entry.
The dashboards display information about the entity, including notes, and relationships with other entities.
...
The fields of the dashboard are dynamically generated based on the entity type, and can include:
- General information about the entity, such as name, description, and type.
- A list of cases directly linked to the entity.
- A list of actors directly linked to the entity.
- A list of entries directly linked to the entity.
- A list of notes that mention the entity.

If there are cases related to the entity to which the user does not have [access](#access-control), a text giving options to request access appears.
...
Admins additionally have access to a button for deleting the entity on case and actor dashboards.

Clicking on a case, actor, or entry in the dashboard will redirect the user to the respective dashboard of the selected entity.
Clicking on a note in the dashboard will open the note in the preview mode, allowing the user to read the note content.


### Graph Visualization
The graph visualization feature allows users to visualize the relationships between cases, actors, and entries in the system.
The graph is interactive and allows users to explore the connections between entities.
The graph can be zoomed in and out, and panned to explore the connections between entities.

Clicking on a node in the graph will display a card (on the top left of the screen) with information about the entity, and a link to the [entity dashboard](#dashboards).
...
- Filter the graph by entity by label.
...
The search feature allows users to search for specific cases, actors, entries in the system.
...
- Type the name of the entity you want to search for in the search bar in the dialog.
- Optionally, you can filter the search results by entity type (case, actor, entry) and entry type. To do this click on the filters button in the search dialog to expand the filter section.
- Click on the `Search` icon in the dialog or press `enter` to perform the search.
- Click on a search result to redirect to the respective dashboard of the selected entity.

The search results are displayed in a list format, with the name of the entity and the type of entity.
Clicking on a search result will redirect the user to the respective [dashboard](#dashboards) of the selected entity.
...
There are notifications for when certain users request access to cases. All users wit `Read-Write` access on those cases receive these notifications.
Users with `Read-Write` access to a case can allow access to cases from notifications by clicking on one of the `Read` or `Read-Write` buttons in the notification.


### Publishing Reports

CRADLE provides a feature for generating and exporting reports based on the information stored in the system.
Reports can be generated for cases, actors, and entries, and include information such as notes, relationships, and attachments.
Reports can be generated only using the contents of [publishable notes](#publishable-and-non-publishable-notes).

To create a report, follow these steps:
- Click on the `Enter Publish Mode` button on the [dashboard](#dashboards) of the entity you want to base the report on
...
The users can also change the publishable status of a note at any time by clicking on the `Publishable` switch in the note card in an entity dashboard.

Only publishable notes can be included in the reports generated by the system.

### Access Control

The system provides a strong access control system that allows administrators and trusted users to manage permissions and access levels on a case-by-case basis.

**Users can only see notes related to cases they have access to. If a note mentions a case the user does not have access to, the note will be hidden from the user.**

The access control system includes the following access levels:
- `None` - Users with no access to the case. They cannot view or interact with the case in any way. They can only request access to the case if informed of its existence.
- `Read`: Users with read access can view the case, actors, and entries linked to the case, as well as the notes that mention the case.
- `Read-Write`: Users with read-write access have the same permissions as users with read access, but can also create and edit notes related to the case.
- `Admin`: Users with admin access have full control over the case, including the ability to manage access levels, delete the case, and manage notes.

The access control system is managed by the system administrator, who can assign access levels to users on a case-by-case basis.
Users can request access to cases they do not have access to by clicking on the `Request Access` button on the case [dashboard](#dashboards).
The request will be sent as a [notification](#notifications) to the users with `Read-Write` or `Admin` access to the case, who can approve or deny the request.

### Admin Controls

Users with admin access have access to additional controls in the application, including:
- The ability to create new cases and actors.
- The ability to delete cases and actors.
- The ability to manage [access levels](#access-control) for cases.
...
The admin page provides a list of cases, actors, and users in the system, as well as controls for creating new cases and actors, and managing access levels and user permissions.
Searching for a specific case, actor, or user can be done by using the search bar in the respective column of the admin page.
Deleting a case, actor, or user can be done by clicking on the `Delete` button next to the entity in their list.
Managing access level for a case can be done on a per-user basis by clicking on the user's card and then selecting the desired access level for the case in the list of cases.
