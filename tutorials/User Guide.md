...
done by allowing analysts to create and share notes referencing specific cases and artifacts
of interest.

The application automatically links the artifacts and cases mentioned in the
notes, making it easier to navigate and understand the context of the information.

CRADLE allows visualizing the relationships between cases and artifacts through informative
...
that toggles the display of the fleeting notes panel, and a [search bar](#search) that allows users to search for specific cases, artifacts, or notes.
...
CRADLE allows users to create and manage notes related to specific cases and artifacts.

When a note is created, the system automatically links the cases, actors, and artifacts mentioned in the note.
...
The note syntax is based on the [original markdown specification](https://daringfireball.net/projects/markdown/syntax) with some additional features to support linking to cases, actors and artifacts.
The syntax for linking to cases, actors and artifacts is as follows:
...
Files uploaded to notes are not linked to cases, actors or artifacts, but can be referenced in the note text.
...
-   The fleeting notes are not linked to cases, actors or artifacts and are not visible to other users.
...
Dashboards are pages that provide all the aggregated data related to a specific entry, such as a case, actor, or artifact.
...
-   A list of cases directly linked to the entry.
...
If there are cases related to the entry to which the user does not have [access](#access-control), a text giving options to request access appears.
...
Admins additionally have access to a button for deleting the entry on case and actor dashboards.

Clicking on a case, actor, or artifact in the dashboard will redirect the user to the respective dashboard of the selected entry.
Clicking on a note in the dashboard will open the note in the preview mode, allowing the user to read the note content.

### Graph Visualization

The graph visualization feature allows users to visualize the relationships between cases, actors, and artifacts in the system.
...
The search feature allows users to search for specific cases, actors, artifacts in the system.
...
-   Optionally, you can filter the search results by entry type (case, actor, artifact) and artifact type. To do this click on the filters button in the search dialog to expand the filter section.
...
Reports can be generated for cases, actors, and artifacts, and include information such as notes, relationships, and attachments.
...
-   `Read`: Users with read access can view the case, actors, and artifacts linked to the case, as well as the notes that mention the case.
...
Deleting a case, actor, or user can be done by clicking on the `Delete` button next to the entry in their list.
