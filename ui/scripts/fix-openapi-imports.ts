...
`;
  ...
  if (matches.length > 0) {
    ...
    modified = modified.slice(0, lastMatch.index) + "default: return value;" + modified.slice(lastMatch.index + lastMatch[0].length);
  }
...
async function fixEntryCompressedTreeValue(): Promise<void> {
  ...
  const hasImport = /import\s*\{[^}]*EntryCompressedTreeObjectFromJSONTyped[^}]*\}\s*from/.test(content);
