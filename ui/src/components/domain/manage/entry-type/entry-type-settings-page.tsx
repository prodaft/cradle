...
const defaultEntryTypeSettingsTabId = ENTRY_TYPE_SETTINGS_ITEMS[0]?.id ?? 'settings';
...
export default function EntryTypeSettingsPage() {
    ...
    const tab = (search as any)?.tab ?? defaultEntryTypeSettingsTabId;
    ...
    const currentTab =
        ...
        ENTRY_TYPE_SETTINGS_ITEMS.find(
            (item) => item.id === defaultEntryTypeSettingsTabId,
        );
