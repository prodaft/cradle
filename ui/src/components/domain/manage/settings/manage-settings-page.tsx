...
const VALID_TAB_IDS = new Set(
    MANAGE_SETTINGS_ITEMS.map((item) => item.id),
) as Set<string>;

export default function ManageSettingsPage() {
    useDockPanelTab({ title: 'Manage: Settings', icon: 'manage-settings' });
    ...
    const ActiveSettingsComponent = tab ? MANAGE_SETTINGS_COMPONENTS[tab] : null;
    const currentTab =
        MANAGE_SETTINGS_ITEMS.find((item) => item.id === tab) ||
        MANAGE_SETTINGS_ITEMS[0];
