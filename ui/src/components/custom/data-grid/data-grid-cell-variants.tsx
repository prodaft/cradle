                ...
                if (!skipUpload) {
                    ...
                    if (tableMeta?.onFilesUpload) {
                        try {
                            ...
                            setFiles((prev) =>
                                prev.filter((f) => !uploadingIds.has(f.id)),
                            );
                    ...
                    const finalFiles = filesWithTemp
                        .map((f) => {
                            if (uploadingIds.has(f.id)) {
                                return (
                                    uploadedFiles.find((uf) => uf.name === f.name) ?? f
                                );
                            }
                            return f;
                        })
                                ...
                                <div className='flex flex-col gap-2'>
                                    ...
                                    <div className='max-h-[200px] space-y-1 overflow-y-auto'>
                                        {files.map((file) => {
                                            ...
                                            const isFileUploading = uploadingFiles.has(
                                                file.id,
                                            );
                                            const isFileDeleting = deletingFiles.has(
                                                file.id,
                                            );
                                            const isFilePending =
