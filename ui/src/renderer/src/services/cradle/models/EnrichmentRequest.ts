/* tslint:disable */
/* eslint-disable */
/**
 * CRADLE
 * Threat Intelligence Knowledge Management
 *
 * The version of the OpenAPI document: 2.10.2-beta.eefb782a (2.10.2-beta.eefb782a)
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
import type { EssentialUserRetrieve } from './EssentialUserRetrieve';
import {
    EssentialUserRetrieveFromJSON,
    EssentialUserRetrieveFromJSONTyped,
    EssentialUserRetrieveToJSON,
    EssentialUserRetrieveToJSONTyped,
} from './EssentialUserRetrieve';
import type { Entry } from './Entry';
import {
    EntryFromJSON,
    EntryFromJSONTyped,
    EntryToJSON,
    EntryToJSONTyped,
} from './Entry';

/**
 * Serializer for enrichment requests.
 * @export
 * @interface EnrichmentRequest
 */
export interface EnrichmentRequest {
    /**
     * 
     * @type {number}
     * @memberof EnrichmentRequest
     */
    readonly id?: number;
    /**
     * 
     * @type {string}
     * @memberof EnrichmentRequest
     */
    title: string;
    /**
     * 
     * @type {Date}
     * @memberof EnrichmentRequest
     */
    readonly createdAt?: Date;
    /**
     * 
     * @type {Date}
     * @memberof EnrichmentRequest
     */
    readonly completedAt?: Date | null;
    /**
     * * `waiting` - Waiting
     * * `working` - Working
     * * `warning` - Warning
     * * `done` - Done
     * * `error` - Error
     * @type {string}
     * @memberof EnrichmentRequest
     */
    readonly status?: EnrichmentRequestStatusEnum;
    /**
     * The user who created the request
     * @type {string}
     * @memberof EnrichmentRequest
     */
    readonly user?: string;
    /**
     * 
     * @type {EssentialUserRetrieve}
     * @memberof EnrichmentRequest
     */
    readonly userDetail?: EssentialUserRetrieve;
    /**
     * The entity to enrich
     * @type {number}
     * @memberof EnrichmentRequest
     */
    entity: number;
    /**
     * 
     * @type {Entry}
     * @memberof EnrichmentRequest
     */
    readonly entityDetail?: Entry;
    /**
     * The enrichment settings used for this request
     * @type {string}
     * @memberof EnrichmentRequest
     */
    readonly enrichmentSettings?: string;
    /**
     * 
     * @type {string}
     * @memberof EnrichmentRequest
     */
    readonly enricherClass?: string;
    /**
     * 
     * @type {string}
     * @memberof EnrichmentRequest
     */
    readonly enricherName?: string;
    /**
     * 
     * @type {any}
     * @memberof EnrichmentRequest
     */
    request?: any | null;
    /**
     * 
     * @type {any}
     * @memberof EnrichmentRequest
     */
    readonly errors?: any | null;
}


/**
 * @export
 */
export const EnrichmentRequestStatusEnum = {
    Waiting: 'waiting',
    Working: 'working',
    Warning: 'warning',
    Done: 'done',
    Error: 'error'
} as const;
export type EnrichmentRequestStatusEnum = typeof EnrichmentRequestStatusEnum[keyof typeof EnrichmentRequestStatusEnum];


/**
 * Check if a given object implements the EnrichmentRequest interface.
 */
export function instanceOfEnrichmentRequest(value: object): value is EnrichmentRequest {
    if (!('title' in value) || value['title'] === undefined) return false;
    if (!('entity' in value) || value['entity'] === undefined) return false;
    return true;
}

export function EnrichmentRequestFromJSON(json: any): EnrichmentRequest {
    return EnrichmentRequestFromJSONTyped(json, false);
}

export function EnrichmentRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): EnrichmentRequest {
    if (json == null) {
        return json;
    }
    return {
        
        'id': json['id'] == null ? undefined : json['id'],
        'title': json['title'],
        'createdAt': json['created_at'] == null ? undefined : (new Date(json['created_at'])),
        'completedAt': json['completed_at'] == null ? undefined : (new Date(json['completed_at'])),
        'status': json['status'] == null ? undefined : json['status'],
        'user': json['user'] == null ? undefined : json['user'],
        'userDetail': json['user_detail'] == null ? undefined : EssentialUserRetrieveFromJSON(json['user_detail']),
        'entity': json['entity'],
        'entityDetail': json['entity_detail'] == null ? undefined : EntryFromJSON(json['entity_detail']),
        'enrichmentSettings': json['enrichment_settings'] == null ? undefined : json['enrichment_settings'],
        'enricherClass': json['enricher_class'] == null ? undefined : json['enricher_class'],
        'enricherName': json['enricher_name'] == null ? undefined : json['enricher_name'],
        'request': json['request'] == null ? undefined : json['request'],
        'errors': json['errors'] == null ? undefined : json['errors'],
    };
}

export function EnrichmentRequestToJSON(json: any): EnrichmentRequest {
    return EnrichmentRequestToJSONTyped(json, false);
}

export function EnrichmentRequestToJSONTyped(value?: Omit<EnrichmentRequest, 'id'|'created_at'|'completed_at'|'status'|'user'|'user_detail'|'entity_detail'|'enrichment_settings'|'enricher_class'|'enricher_name'|'errors'> | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'title': value['title'],
        'entity': value['entity'],
        'request': value['request'],
    };
}

