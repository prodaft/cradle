...
const setCradleTab = (dt: DataTransfer, payload: DragData): void => dt.setData(DND_MIME, JSON.stringify(payload));
...
const hasCradleTab = (dt: DataTransfer | null): boolean => {
    if (!dt) return false;
    const t = dt?.types || [];
    return (typeof (t as any).contains === 'function' ? (t as any).contains(DND_MIME) : Array.from(t).includes(DND_MIME)) || !!dt?.getData(DND_MIME);
};
 ...
 */
const computeReorderTarget = ({ sourceIndex, hoverIndex, dropBefore }: { sourceIndex: number; hoverIndex: number; dropBefore: boolean }): number => {
...
const PaneTabs = ({ paneId, isActive, onRootRef }: PaneTabsProps) => {
    ...
    const handleCloseClick = useCallback((e: React.MouseEvent, index: number) => {
        e.stopPropagation();
    ...
    return (
        ...
        >
            ...
            <button
                ...
                onMouseEnter={(e) => (e.target as HTMLElement).style.borderColor = 'var(--cradle-accent-primary)'}
                onMouseLeave={(e) => (e.target as HTMLElement).style.borderColor = 'var(--cradle-border-primary)'}
...
const LayoutPane = ({ paneId, outletContext }: LayoutPaneProps) => {
    ...
    const pickZoneSticky = (zone: DropZone, e: DragEvent | React.DragEvent): DropZone => {
                        ...
                        <div
                            ...
                            ref={(el) => {
                                if (el) {
                                    ...
                                    if (!LayoutPaneStatic.attachedTabs.has(attachmentKey)) {
                                        console.log('[LayoutPane] First attachment of tab:', tab.id, 'to pane:', paneId);
                                        LayoutPaneStatic.attachedTabs.add(attachmentKey);
                                    }
                                    requestAnimationFrame(() => {
