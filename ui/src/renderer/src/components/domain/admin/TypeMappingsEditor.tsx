...
const TypeMappingsEditor = ({ id }: TypeMappingsEditorProps) => {
  ...
  const handleSaveAll = () => {
    ...
    rows.forEach((row, rowIndex) => {
      ...
      if ((rowIndex === rows.length - 1 && !row.edited) || !row.edited)
        return;
            ...
            <tbody>
              {rows.map((row, index) => (
                <tr
                  key={index}
                  className={
                    index < rows.length - 1 && !row.edited ? 'bg-gray-1' : ''
                  }
                >
                  ...
                  {allColumns.map((column) => {
                    ...
                    if (colType === 'options') {
                      return (
                        <td key={`${index}-${column}`} className="px-2 py-2">
                          <Selector
                            value={row[column]}
                            onChange={(option) =>
                              handleCellChange(index, column, option)
                            }
                            staticOptions={
                            ...
                            placeholder={
                              colDef.required ? 'Required...' : 'Select...'
                            }
