...
import { EnrichmentRelation, EnrichmentRequestDetail, EnrichmentRequestDetailStatusEnum, EntrySerializerMinimal } from '@services/cradle/models';
import { Calendar, CheckCircle, Clock, InfoCircle, User, WarningCircle, WarningTriangle } from 'iconoir-react';
...
export default function EnrichmentResults(): JSX.Element {
  ...
  const [enrichmentDetails, setEnrichmentDetails] = useState<EnrichmentRequestDetail | null>(null);
  ...
  const getEnricherStatusIcon = (enricherType: string) => {
    ...
    switch (status) {
      case 'done':
        return (
          <CheckCircle className='text-green-500 flex-shrink-0' width='16' height='16' />
        );
      case 'working':
      case 'waiting':
        return <InfoCircle className='text-blue-500 flex-shrink-0' width='16' height='16' />;
      case 'warning':
        return (
          <WarningTriangle
      ...
      case 'error':
        return (
          <WarningCircle className='text-red-500 flex-shrink-0' width='16' height='16' />
        );
      default:
        return null;
    }
            ...
            <div className='flex items-center gap-4 text-xs cradle-text-tertiary'>
              {enrichmentDetails.status && (
                <div className='flex items-center gap-1.5'>
                  ...
                  <span className='capitalize'>{enrichmentDetails.status}</span>
                </div>
              )}
                  ...
                  <div className='space-y-1 pr-2'>
                    {enrichersDetail.map((enricher) => (
                      <div
                        ...
                        className={`p-2 hover:border-2 hover:border-cradle-accent-primary cursor-pointer transition-colors ${selectedEnricher === enricher.enricher_type
                          ? 'cradle-bg-accent border-2 border-cradle-border-accent'
                          : 'cradle-bg-base'
                          }`}
                        onClick={() => {
                          setSelectedEnricher(enricher.enricher_type);
                          setPage(1);
                      ...
                      >
                        <div className='flex items-center gap-2'>
                          {getEnricherStatusIcon(enricher.enricher_type)}
                  ...
                  {enricherWarning && (
                    <div className='pt-3'>
                      <AlertBox
                        alert={{
                          show: true,
                          message: typeof enricherWarning === 'string'
                            ? enricherWarning
                            : JSON.stringify(enricherWarning),
                          color: 'warning',
                        }}
                      />
                    </div>
