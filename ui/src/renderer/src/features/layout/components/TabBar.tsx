...
import { Tab, TabDragData, PaneId, TAB_DRAG_TYPE } from '../types';
...
const TabItem = memo(function TabItem({
    ...
    return (
        <div
            ...
            `}
            style={isActive && isPaneActive ? { borderTopColor: 'var(--cradle-accent-primary)' } : {}}
        ...
        >
            ...
            {tabIndex < tabCount - 1 && (
                <button
                    role="menuitem"
                    className="w-full px-4 py-2 text-left text-sm cradle-text-secondary hover:cradle-bg-secondary flex items-center gap-2"
                    onClick={() => { onCloseToRight(); onClose(); }}
                >
...
export function TabBar({ paneId, tabs, activeTabIndex, isPaneActive }: TabBarProps) {
    const { closeTab, switchTab, reorderTabs, moveTabToPane, closeOtherTabs, closeTabsToRight, openTab, setActivePaneId } = useLayout();
