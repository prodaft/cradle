...
import { PaneId, DropZone, TabDragData, TAB_DRAG_TYPE } from '../types';
...
function DropZoneOverlay({ zone, tabBarHeight }: DropZoneOverlayProps) {
    ...
    switch (zone) {
        case 'top':
            return <div style={{ ...style, top: tabBarHeight, left: 0, right: 0, height: '50%' }} />;
        case 'bottom':
            return <div style={{ ...style, bottom: 0, left: 0, right: 0, height: '50%' }} />;
        case 'left':
            return <div style={{ ...style, top: tabBarHeight, bottom: 0, left: 0, width: '50%' }} />;
        case 'right':
            return <div style={{ ...style, top: tabBarHeight, bottom: 0, right: 0, width: '50%' }} />;
        default:
            return null;
    }
}
...
export function Pane({ paneId }: PaneProps) {
    const { state, setActivePaneId, getPaneState, splitPane, moveTabToPane } = useLayout();
    ...
    const calculateDropZone = useCallback((e: DragEvent | React.DragEvent): DropZone => {
    ...
    const handleDrop = useCallback((e: React.DragEvent) => {
        ...
        try {
            const data = JSON.parse(e.dataTransfer.getData(TAB_DRAG_TYPE)) as TabDragData;
    ...
    return (
        ...
        >
            {/* Drop zone overlay */}
            {showOverlay && <DropZoneOverlay zone={dropZone} tabBarHeight={tabBarHeight} />}
