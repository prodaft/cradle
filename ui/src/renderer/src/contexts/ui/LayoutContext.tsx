...
const removeNode = (node: LayoutNode, targetId: string): LayoutNode | null => {
    ...
    if (remaining.length === 2) {
        ...
        return {
            ...node,
            children: [remaining[0], remaining[1]] as [LayoutNode, LayoutNode],
        };
    }
...
const reducer = (state: LayoutState, action: Action): LayoutState => {
    switch (action.type) {
        case 'SPLIT_PANE': {
            ...
            const { next, newPaneId } = splitLeaf(
                state.root,
                paneId,
                orientation,
                position,
...
type LayoutContextValue = {
    ...
    splitPane: (
        ...
        position?: SplitPosition,
    ) => { originalPaneId: PaneId; newPaneId?: PaneId };
...
export const LayoutProvider = ({ children }: { children: ReactNode }) => {
    ...
    const updatePaneSizes = useCallback<LayoutContextValue['updatePaneSizes']>(
        (containerId, newSizes) =>
            dispatch({ type: 'UPDATE_SIZES', containerId, sizes: newSizes }),
        [],
    );
