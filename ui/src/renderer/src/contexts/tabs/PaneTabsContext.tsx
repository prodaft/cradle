...
interface PaneTabsContextValue {
    paneTabsState: PaneTabsState;
    openTab: (paneId: string, path: string) => void;
    closeTab: (paneId: string, index: number) => void;
    switchToTab: (paneId: string, index: number) => void;
    closeOtherTabs: (paneId: string, index: number) => void;
    closeTabsToRight: (paneId: string, index: number) => void;
    reorderTabs: (paneId: string, fromIndex: number, toIndex: number) => void;
    updateCurrentTabTitle: (paneId: string, title: string) => void;
    createNewTab: (paneId: string) => void;
...
const PaneTabsContext = createContext<PaneTabsContextValue | undefined>(undefined);

/**
 * Custom hook to access the PaneTabsContext
 */
export const usePaneTabs = (): PaneTabsContextValue => {
...
export const PaneTabsProvider = ({ children }: PaneTabsProviderProps) => {
     ...
     */
    const initializePaneIfNeeded = useCallback(
        (paneId: string, initialPath?: string) => {
    ...
    const switchToTab = useCallback(
        (paneId: string, index: number) => {
            ...
            setPaneTabsState((current) => {
                ...
                if (
                    !paneState ||
                    !validatePaneState(paneState) ||
                    index < 0 ||
                    index >= paneState.tabs.length
                ) {
                    return current;
                }
     ...
     */
    const getPaneTabsState = useCallback(
        (paneId: string): PaneState => {
     ...
     */
    const updateCurrentTabTitle = useCallback((paneId: string, title: string) => {
        setPaneTabsState((current) => {
            const paneState = current[paneId];
            if (!paneState || !validatePaneState(paneState)) {
                return current;
    ...
    const moveTabBetweenPanes = useCallback(
        (fromPaneId: string, fromIndex: number, toPaneId: string, toIndex = -1) => {
     ...
     */
    const transferTabs = useCallback(
        (fromPaneId: string, toPaneId: string, removeSource = true) => {
            setPaneTabsState((current) => {
                const fromPaneState = current[fromPaneId];
                const toPaneState = current[toPaneId] || createPaneState();
     ...
     */
    const preInitializePanes = useCallback((paneIds: string[]) => {
        setPaneTabsState((current) => {
    ...
    const contextValue = useMemo<PaneTabsContextValue>(
        () => ({
            paneTabsState,
            openTab,
            closeTab,
